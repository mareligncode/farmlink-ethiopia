...
const AddProduct: React.FC = () => {
  ...
  const createProductMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: t('message.success'),
        description: language === 'am' ? 'ምርት ተጨምሯል' : 'Product added successfully',
      });
      navigate('/products');
    },
    onError: (error) => {
    ...
    <div className="min-h-screen bg-background pb-8">
      ...
      <div className="bg-card border-b border-border px-6 py-4 sticky top-0 z-10 safe-area-top">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold">{t('product.add')}</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="px-6 py-6 space-y-6">
        ...
        >
          {createProductMutation.isPending ? t('action.loading') : t('action.save')}
        </Button>
      </form>
    </div>
  );
