    ...
    <div className="min-h-screen bg-background pb-8">
      ...
      <div className="px-6 py-6 space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-primary/10"><DollarSign className="h-5 w-5 text-primary" /></div><div><p className="text-sm text-muted-foreground">{language === 'am' ? 'ጠቅላላ ገቢ' : 'Total Revenue'}</p><p className="text-xl font-bold">{analytics.summary.totalRevenue.toLocaleString()} ETB</p></div></div></CardContent></Card>
          <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-secondary/10"><ShoppingBag className="h-5 w-5 text-secondary" /></div><div><p className="text-sm text-muted-foreground">{language === 'am' ? 'ጠቅላላ ትዕዛዞች' : 'Total Orders'}</p><p className="text-xl font-bold">{analytics.summary.totalOrders}</p></div></div></CardContent></Card>
          <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-accent/10"><Package className="h-5 w-5 text-accent" /></div><div><p className="text-sm text-muted-foreground">{language === 'am' ? 'ምርቶች' : 'Products'}</p><p className="text-xl font-bold">{analytics.summary.availableProducts}/{analytics.summary.totalProducts}</p></div></div></CardContent></Card>
          <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-yellow-500/10"><Star className="h-5 w-5 text-yellow-500" /></div><div><p className="text-sm text-muted-foreground">{language === 'am' ? 'አማካይ ደረጃ' : 'Avg Rating'}</p><p className="text-xl font-bold">{analytics.summary.avgRating}/5 ({analytics.summary.totalReviews})</p></div></div></CardContent></Card>
        </div>
        ...
        {analytics.categoryBreakdown.length > 0 && (
          <Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={200}>
                <RechartPie>
                  <Pie data={analytics.categoryBreakdown} cx="50%" cy="50%" innerRadius={40} outerRadius={80} dataKey="revenue" nameKey="category" label={({ category, percent }) => `${(categoryLabels[category]?.[language] || category)}: ${(percent * 100).toFixed(0)}%`} labelLine={false}>
                    {analytics.categoryBreakdown.map((_, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}
                  </Pie>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-3">
              {Object.entries(analytics.ordersByStatus).map(([status, count]) => (<div key={status} className="text-center p-3 bg-muted/50 rounded-lg"><p className="text-lg font-bold">{count}</p><p className="text-xs text-muted-foreground capitalize">{status}</p></div>))}
            </div>
          </CardContent>
        </Card>
